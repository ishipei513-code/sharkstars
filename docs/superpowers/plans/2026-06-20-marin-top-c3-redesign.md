...
## ⚠ デザイン改定（2026-06-20 ユーザー指示・最優先）

実装中に出た指示。**以降の全タスクで厳守**：

- **AI的グラデーション禁止**：背景・テキスト・バー等の color gradient（`linear-gradient`/`radial-gradient` の色ブレンド）は使わず**単色フラット**にする。plan内のコードにグラデがあれば単色へ置換して実装：
  - 数字バー `linear-gradient(90deg,var(--sea),var(--pink))` → `var(--pink)`（Task 7 `.num-ratio-f`）
  - セクション背景の `radial-gradient(...)`（Task 7 `.numbers` / Task 8 `.recruit`）→ 単色 `var(--navy-1)` 等
  - 見出し下線のグラデ（Task 5 `.phi-key::after`）→ 単色 `var(--pink)`
- **絵文字禁止**：🩺🌿 等の絵文字や ♡ ＋ ✦ 等の記号は使わない（Task 5 `.route-ic` / Task 6 看護・リハ見出し）。アイコンが要る所は**クリーンな線SVG**（stroke系）か、アイコン無し。
- **青＝#005aaa**（`--navy-2`）。メリハリは「単色ブロックのコントラスト」で出す（グラデで出さない）。
- ヒーローは適用済み（Task 3＝フラット化・#005aaa・ボタン中央「応募」・隙間なしリング・回転24秒）。
